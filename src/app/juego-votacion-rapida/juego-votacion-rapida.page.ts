...
export class JuegoVotacionRapidaPage implements OnInit {
  ...
  reorderItems(event) {
    const itemMove = this.juegoSeleccionado.Conceptos.splice(
      event.detail.from,
      1,
    )[0];
  ...
  async EnviarVotacion() {
    ...
    this.peticionesAPI
      .DameJuegoDeVotacionRapida(this.juegoSeleccionado.Clave)
      .subscribe((juego) => {
        juego[0].Respuestas.push({ nick: this.nickName, votos: this.misVotos });
