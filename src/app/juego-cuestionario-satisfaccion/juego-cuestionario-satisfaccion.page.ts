...
export class JuegoCuestionarioSatisfaccionPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.peticionesAPI
      .DameCuestionarioSatisfaccion(
        this.juegoSeleccionado.cuestionarioSatisfaccionId,
      )
    ...
    if (
      this.juegoSeleccionado.Tipo === "Juego De Cuestionario de Satisfacción"
    ) {
      ...
      this.peticionesAPI
        .DameInscripcionAlumnoJuegoDeCuestionarioSatisfaccion(
          this.juegoSeleccionado.id,
          this.alumno.id,
        )
        .subscribe((inscripcion) => {
          this.inscripcionAlumnoJuegoDeCuestionarioSatisfaccion =
            inscripcion[0];
          console.log("ya tengo la inscripcion");
          console.log(this.inscripcionAlumnoJuegoDeCuestionarioSatisfaccion);
          if (
            !this.inscripcionAlumnoJuegoDeCuestionarioSatisfaccion.Contestado
          ) {
  ...
  Avanzar(indice: number) {
    ...
    if (
      indice >= 1 &&
      indice <= this.cuestionarioSatisfaccion.Afirmaciones.length
    ) {
  ...
  Retroceder(indice: number) {
    ...
    if (
      indice >= 2 &&
      indice <= this.cuestionarioSatisfaccion.Afirmaciones.length + 1
    ) {
  ...
  async EnviarRespuesta() {
    ...
    // me lo traje
    this.peticionesAPI
      .DameJuegoDeEncuestaRapida(this.juegoSeleccionado.Clave)
      .subscribe((juego) => {
  ...
  Registrar() {
    ...
    this.peticionesAPI
      .ModificaInscripcionAlumnoJuegoDeCuestionarioSatisfaccion(
        this.inscripcionAlumnoJuegoDeCuestionarioSatisfaccion,
      )
      .subscribe(async () => {
        const confirm = await this.alertCtrl.create({
  ...
  async MuestraPicker(i: number) {
    ...
    if (
      this.inscripcionAlumnoJuegoDeCuestionarioSatisfaccion
        .RespuestasAfirmaciones[i] !== undefined
    ) {
      this.opcionesPicker.columns[0].selectedIndex =
        this.inscripcionAlumnoJuegoDeCuestionarioSatisfaccion
          .RespuestasAfirmaciones[i] - 1;
    }
    ...
    picker.onDidDismiss().then(async (data) => {
      if (this.pickerAction === "done") {
        ...
        console.log(
          "pongo en " + i + "el valor " + col.options[col.selectedIndex].value,
        );
        this.inscripcionAlumnoJuegoDeCuestionarioSatisfaccion.RespuestasAfirmaciones[
          i
