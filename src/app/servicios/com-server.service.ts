...
export class ComServerService {
  ...
  EnviarNick(profesorId: number, nick: string) {
    ...
    this.servidor.emit("nickNameJuegoRapido", {
      profesorId: this.profesorId,
      info: nick,
    });
  }
  ...
  EnviarNickYRegistrar(profesorId: number, nick: string, clave: string) {
    ...
    this.profesorId = profesorId;
    // tslint:disable-next-line:object-literal-shorthand
    this.servidor.emit("nickNameJuegoRapidoYRegistro", {
      profesorId: this.profesorId,
      info: nick,
      c: clave,
    });
  ...
  public RecordarContrasena(alumno: Alumno) {
    ...
    this.servidor.connect();
    this.servidor.emit("recordarContraseña", {
      email: alumno.Email,
      nombre: alumno.Username,
      contrasena: alumno.Password,
    });
    // Me desconecto
    this.servidor.disconnect();
  }
  ...
  public EnviarRespuestaKahoot(alumnoId: number, respuestaEscogida: string) {
    ...
    this.servidor.emit("respuestaAlumnoKahoot", {
      alumnoId: alumnoId,
      respuesta: respuestaEscogida,
      profesorId: this.profesorId,
    });
  }
  ...
  public EnviarConexionAlumnoKahoot(alumnoId: number) {
    ...
    this.servidor.emit("conexionAlumnoKahoot", {
      alumnoId: alumnoId,
      profesorId: this.profesorId,
    });
  }
