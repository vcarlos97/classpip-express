...
export class JuegoDeCuestionarioPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.juegoSeleccionado.Tipo === "Juego De Cuestionario") {
      ...
      this.peticionesAPI
        .DameInscripcionAlumnoJuegoDeCuestionario(
          this.alumnoId,
        ...
        .subscribe((res) => {
          ...
          if (!this.alumnoJuegoDeCuestionario.Contestado) {
            ...
            this.peticionesAPI
              .DameCuestionario(this.juegoSeleccionado.cuestionarioId)
            ...
            this.peticionesAPI
              .DamePreguntasCuestionario(this.juegoSeleccionado.cuestionarioId)
              // tslint:disable-next-line:no-shadowed-variable
              .subscribe((res) => {
                this.seleccion = [];
                this.PreguntasCuestionario = res;
                this.contestar = Array(this.PreguntasCuestionario.length).fill(
                  true,
                );
                ...
                if (
                  this.juegoSeleccionado.Presentacion ===
                ...
                for (let i = 0; i < this.preguntasYRespuestas.length; i++) {
                  ...
                  this.imagenesPreguntas[i] =
                    URL.ImagenesPregunta +
                    this.preguntasYRespuestas[i].pregunta.Imagen;
                }
              });
          } else {
            ...
            this.peticionesAPI
              ...
              .subscribe((res) => {
                ...
                this.puntuacionMaxima =
                  this.puntuacionCorrecta * this.PreguntasCuestionario.length;

                this.peticionesAPI
                  .DameRespuestasAlumnoJuegoDeCuestionario(
                    this.alumnoJuegoDeCuestionario.id,
                  )
                  .subscribe((respuestas) => {
                    ...
                    for (
                      let i = 0;
                      i < this.PreguntasCuestionario.length;
                      i++
                    ) {
                      this.imagenesPreguntas[i] =
                        URL.ImagenesPregunta +
                        this.PreguntasCuestionario[i].Imagen;
                      if (
                        this.PreguntasCuestionario[i].Tipo === "Emparejamiento"
                      ) {
                        // tslint:disable-next-line:max-line-length
                        this.respuestasEmparejamientos[i] = respuestas.filter(
                          (r) =>
                            r.preguntaId === this.PreguntasCuestionario[i].id,
                        )[0].Respuesta;
                        if (this.respuestasEmparejamientos[i] === undefined) {
                          this.contestar[i] = false;
                          this.feedbacks.push(
                            this.PreguntasCuestionario[i].FeedbackIncorrecto,
                          );
                        } else {
                          ...
                          for (
                            let j = 0;
                            j <
                            this.PreguntasCuestionario[i].Emparejamientos
                              .length;
                            j++
                          ) {
                            if (
                              this.PreguntasCuestionario[i].Emparejamientos[j]
                                .r === this.respuestasEmparejamientos[i][j]
                            ) {
                              cont++;
                            }
                          }
                          if (
                            cont ===
                            this.PreguntasCuestionario[i].Emparejamientos.length
                          ) {
                            this.feedbacks.push(
                              this.PreguntasCuestionario[i].FeedbackCorrecto,
                            );
                          } else {
                            this.feedbacks.push(
                              this.PreguntasCuestionario[i].FeedbackIncorrecto,
                        ...
                        // tslint:disable-next-line:max-line-length
                        this.RespuestasAlumno[i] = respuestas.filter(
                          (respuesta) =>
                            respuesta.preguntaId ===
                            this.PreguntasCuestionario[i].id,
                        )[0].Respuesta[0];
                        if (
                          this.RespuestasAlumno[i] ===
                          this.PreguntasCuestionario[i].RespuestaCorrecta
                        ) {
                          this.feedbacks.push(
                            this.PreguntasCuestionario[i].FeedbackCorrecto,
                          );
      ...
      this.peticionesAPI
        ...
        .subscribe((res) => {
          ...
          console.log(this.preguntasYRespuestas);
          if (
            this.juegoSeleccionado.Presentacion === "Mismo orden para todos"
          ) {
            this.DesordenarRespuestas();
          } else if (
            this.juegoSeleccionado.Presentacion === "Preguntas desordenadas"
          ) {
            this.DesordenarPreguntas();
          } else {
            console.log("preguntas y respuestas desordenadas");
  ...
  DesordenarRespuestas() {
    this.preguntasYRespuestas.forEach((item) => {
      if (
        item.pregunta.Tipo === "Cuatro opciones" ||
        item.pregunta.Tipo === "Emparejamiento"
      ) {
  ...
  registrarNota() {
    ...
    for (let i = 0; i < this.preguntasYRespuestas.length; i++) {
      if (this.preguntasYRespuestas[i].pregunta.Tipo === "Emparejamiento") {
        const final =
          this.preguntasYRespuestas[i].pregunta.Emparejamientos.length;
        if (!this.contestar[i]) {
          this.feedbacks.push(
            this.preguntasYRespuestas[i].pregunta.FeedbackIncorrecto,
          );
        } else {
          // tslint:disable-next-line:no-shadowed-variable
          let cont = 0;
          for (
            let j = 0;
            j < this.preguntasYRespuestas[i].pregunta.Emparejamientos.length;
            j++
          ) {
            if (
              this.preguntasYRespuestas[i].pregunta.Emparejamientos[j].r ===
              this.preguntasYRespuestas[i].respuestas[j]
            ) {
              cont++;
            }
          }
          if (
            ...
            this.feedbacks.push(
              this.preguntasYRespuestas[i].pregunta.FeedbackCorrecto,
            );
          } else {
        ...
        if (
          this.RespuestasAlumno[i] ===
          this.preguntasYRespuestas[i].pregunta.RespuestaCorrecta
        ) {
    ...
    // tslint:disable-next-line:max-line-length
    this.peticionesAPI
      .PonerNotaAlumnoJuegoDeCuestionario(
    ...
    for (let i = 0; i < this.preguntasYRespuestas.length; i++) {
      ...
      if (
        this.RespuestasAlumno[i] === "" ||
        this.RespuestasAlumno[i] === undefined
      ) {
        this.RespuestasAlumno[i] = "-";
      }
      let respuestas;
      ...
      // tslint:disable-next-line:max-line-length
      this.peticionesAPI
        .GuardarRespuestaAlumnoJuegoDeCuestionario(
          new RespuestaJuegoDeCuestionario(
            this.alumnoJuegoDeCuestionario.id,
            this.preguntasYRespuestas[i].pregunta.id,
            respuestas,
          ),
        )
        .subscribe((res) => {
          ...
          if (cont === this.PreguntasCuestionario.length) {
            ...
            this.comServer.Emitir("respuestaJuegoDeCuestionario", {
              id: this.alumnoId,
              nota: this.Nota,
  ...
  IniciarTimer() {
    if (this.juegoSeleccionado.Modalidad === "Test clásico") {
              ...
              buttons: [
                {
                  ...
                  handler: () => {
                    this.registrado = true;
                    if (
                      this.juegoSeleccionado.Tipo === "Juego de Cuestionario"
                    ) {
                      this.ponerNota();
                    } else {
      ...
      this.comServer.EsperoAvanzarPregunta().subscribe((mensaje) => {
  ...
  async EnviarRespuesta() {
    ...
    */
    ...
    this.preguntasYRespuestas.forEach((item) =>
      preguntas.push(item.pregunta.id),
    );
    let respuesta: any = [];
    respuesta = {
      Nota: this.Nota,
    ...
    // me lo traje
    this.peticionesAPI
      .DameJuegoDeCuestionarioRapido(this.juegoSeleccionado.Clave)
      .subscribe((juego) => {
        ...
        console.log(juego[0]);
        this.peticionesAPI
          .ModificarJuegoDeCuestionarioRapido(juego[0])
          .subscribe();
      });
    this.registrado = true;
  ...
  EstoyConPregunta() {
    if (
      this.slideActual >= 1 &&
      this.slideActual <= this.PreguntasCuestionario.length
    ) {
      return true;
    } else {
      return false;
  ...
  doCheck() {
    if (this.registrado) {
      ...
      this.slides.getActiveIndex().then((index) => {
        ...
        if (
          !this.registrado &&
          index === this.PreguntasCuestionario.length + 2
        ) {
          ...
          index < this.PreguntasCuestionario.length + 1
        ) {
  ...
  AlumnosDelJuego() {
    this.peticionesAPI
      .DameAlumnosJuegoDeCuestionario(this.juegoSeleccionado.id)
  ...
  RecuperarInscripcionesAlumnoJuego() {
    this.peticionesAPI
      .DameInscripcionesAlumnoJuegoDeCuestionario(this.juegoSeleccionado.id)
      .subscribe((inscripciones) => {
        this.listaAlumnosOrdenadaPorNota = inscripciones;
        // tslint:disable-next-line:only-arrow-functions
        this.listaAlumnosOrdenadaPorNota =
          this.listaAlumnosOrdenadaPorNota.sort(function (a, b) {
            if (b.Nota !== a.Nota) {
  ...
  TablaClasificacionTotal() {
    this.rankingAlumnosPorNota = this.calculos.PrepararTablaRankingCuestionario(
      this.listaAlumnosOrdenadaPorNota,
      this.alumnosDelJuego,
    );
  ...
  reorderItems(event, i) {
    ...
    const itemMove = this.preguntasYRespuestas[i].respuestas.splice(
      event.detail.from,
      1,
    )[0];
    ...
    this.preguntasYRespuestas[i].respuestas.splice(
      event.detail.to,
      0,
      itemMove,
    );
